...
public class BlockNightlightBlock extends BlockContainer {
    private boolean nightTime = false;

    protected BlockNightlightBlock(int id) {
        super(id, Material.circuits);
    }

    // This method returns the ID of the item dropped when the block is destroyed
    public int idDropped(int i, Random random, int j) {
        return mod_retrorevisions.NightlightBlock.blockID;
    }
    ...
    public void updateTick(World world, int x, int y, int z, Random random) {
        ...
        boolean newNightTime = time >= 12000 && time < 24000; // Nighttime is from 12000 to 23999 ticks

        if (newNightTime != nightTime) { // Only update if there's a change in daytime
    ...
    private void NightlightSensor(World world, final int x, final int y, final int z) {
