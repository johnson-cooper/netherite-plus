    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, int par2)
    {
        if (!this.worldObj.isRemote && !this.isDead)
        {
            ...
            if (this.getDamageTaken() > 40)
            {
                ...
                {
                    this.dropItemWithOffset(mod_retrorevisions.ItemIronBoat.shiftedIndex, 1, 0.0F);
                }
    ...
    public void onUpdate()
    {
        ...
        if (this.worldObj.isRemote)
        {
            ...
            else
            {
                ...
                if (this.onGround) {
                    int blockBelowID = this.worldObj.getBlockId(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY) - 1, MathHelper.floor_double(this.posZ));
                    if (blockBelowID != Block.ice.blockID) {
                        // Only reduce motion if not on ice
                        this.motionX *= 0.5D;
                        this.motionY *= 0.5D;
                        this.motionZ *= 0.5D;
                    }
        ...
        else
        {
            ...
            if (var16 != null && var16.size() > 0)
            {
                for (var24 = 0; var24 < var16.size(); ++var24)
                {
                    ...
                    if (var18 != this.riddenByEntity && var18.canBePushed() && var18 instanceof EntityIronBoat)
                    {
            ...
            int blockBelowID = this.worldObj.getBlockId(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY) - 1, MathHelper.floor_double(this.posZ));
            if (blockBelowID == Block.ice.blockID) {
                // Set velocity for faster speed on ice
                double iceMultiplier = 10.0D; // Increased multiplier for faster speed on ice
                double newVelocityX = this.motionX * iceMultiplier;
                double newVelocityZ = this.motionZ * iceMultiplier;
                this.setVelocity(newVelocityX, this.motionY, newVelocityZ);
                // Reduce steering sensitivity on ice
                this.rotationYaw *= 0.1F; // Adjust the value as needed
            }
